# /api/index.py
#
# Your original code, adapted with the minimum necessary changes for Vercel.
...
# --- FIX 3: Custom JSON Response to Handle Datetime Objects ---
# This class teaches FastAPI how to convert date/datetime objects to strings,
# which fixes the 'TypeError: Object of type datetime is not JSON serializable' error.
...
# The default_response_class tells all routes to use our custom date-handling logic.
...
# NOTE: load_dotenv() is removed as Vercel injects environment variables automatically.
...
# --- Service Classes (Your Original Code) ---
...
class GeminiManager:
    ...
    # Please ensure the full class definition is in your file.
...
# --- FIX 2: Remove /api prefix from route paths ---
@app.post("/query")
async def handle_query(request: QueryRequest):
    flow, user_query = request.flow, request.query
    logger.info(f"Request received. Flow: {flow}, Query: {user_query}")
    if not user_query:
