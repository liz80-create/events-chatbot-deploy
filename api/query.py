...
# This class teaches the json library how to handle date/datetime objects.
...
# --- Your Application Logic and Classes ---
...
class NeonDBManager:
    ...
    def setup_database(self):
        q = """CREATE TABLE IF NOT EXISTS events (
            ...
            linked_space VARCHAR(255), 
            dependencies TEXT, 
            owner VARCHAR(255), 
            notes TEXT, 
            tags TEXT, 
            pmo_tracking VARCHAR(255), 
            created_on DATE, 
            updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
        ); 
        CREATE INDEX IF NOT EXISTS idx_events_name ON events(name); 
        CREATE INDEX IF NOT EXISTS idx_events_start_time ON events(start_time); 
        CREATE INDEX IF NOT EXISTS idx_events_linked_space ON events(linked_space);"""
    ...
    def sync_records(self, records: List[Dict[str, Any]]):
        ...
        VALUES (%(airtable_id)s, %(name)s, %(source)s, %(workstream)s, %(programme)s, %(type)s, %(start_time)s, %(end_time)s, %(linked_space)s, %(dependencies)s, %(owner)s, %(notes)s, %(tags)s, %(pmo_tracking)s, %(created_on)s) 
        ...
        linked_space = EXCLUDED.linked_space, 
        dependencies = EXCLUDED.dependencies, 
        owner = EXCLUDED.owner, 
        notes = EXCLUDED.notes, 
        tags = EXCLUDED.tags, 
        pmo_tracking = EXCLUDED.pmo_tracking, 
    ...
    def _process_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'start_time': _parse_datetime(fields.get('StartTime')),
            'end_time': _parse_datetime(fields.get('EndTime')),
            'linked_space': fields.get('LinkedSpace'),
...
class GeminiManager:
    ...
    async def generate_sql_query(self, user_query: str, flow: str, table_schema: str) -> str:
        # ... (This class is correct and is omitted for brevity)
...
3. **Split Locations**: For "SSH 3", generate `WHERE (linked_space ILIKE '%SSH%' AND linked_space ILIKE '%3%')`.

4. **Broaden Search**: For "film screenings," create a broad `OR` search for `film` and `screenings` across `name`, `programme`, `notes`, etc.

5. **Parentheses are Mandatory**: You MUST wrap `OR` conditions in parentheses when combining with `AND`. `WHERE (A OR B) AND C`.

6. **Multi-Field Search**: If a specific entity like "Festival Programming" is mentioned, search for it across `workstream`, `programme`, `name`, `owner`.
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        # ... (This section is correct, but needs the fix below)
        ...
        try:
            ...
            # --- FIX: Use the custom encoder when dumping the JSON ---
            self.wfile.write(json.dumps(results, cls=CustomJSONEncoder).encode('utf-8'))
